...
def rejection_sampling_trial(x, j, M):
    ...
    #         U = np.random.uniform()
    #         #print(U, trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x, scale=2.).pdf(Y)))
    #         if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x, scale=2.).pdf(Y)):
    #             return Y
    # elif j < M - 1:
    #     upper_bound = np.max(
    #         [trial_proposal(p, x, j, M) / (norm(loc=x - 4*j, scale=2.).pdf(p) / 2.) for p in np.linspace(x - (j+1)*4, x - (j-1) * 4, 100)])
    #     while True:
    #         U = np.random.uniform()
    #         if np.random.rand() < .5:
    #             Y = norm(loc=x - 4. * j, scale=2.).rvs()
    #             if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x - 4. * j, scale=2.).pdf(Y) / 2.):
    #                 return Y
    #         else:
    #             Y = norm(loc=x + 4. * j, scale=2.).rvs()
    #             if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x + 4. * j, scale=2.).pdf(Y) / 2.):
    ...
    # else:
    #     upper_bound = np.max(
    #         [trial_proposal(p, x, j, M) / (norm(loc=x - 4 * j, scale=3.).pdf(p) / 2.) for p in
    #          np.linspace(x - (j + 3) * 4, x - (j - 3) * 4, 100)])
    #     while True:
    #         U = np.random.uniform()
    #         if np.random.rand() < .5:
    #             Y = norm(loc=x - 4. * j, scale=3.).rvs()
    #             if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x - 4. * j, scale=3.).pdf(Y) / 2.):
    #                 return Y
    #         else:
    #             Y = norm(loc=x + 4. * j, scale=3.).rvs()
    #             if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x + 4. * j, scale=3.).pdf(Y) / 2.):
    #                 return Y
    upper_bound = np.max(
        [trial_proposal(p, x, j, M) / (norm(loc=x - 4*j, scale=5.).pdf(p) / 2.) for p in np.linspace(x - (j+1)*4, x - (j-1) * 4, 100)])
    while True:
        U = np.random.uniform()
        if np.random.rand() < .5:
            Y = norm(loc=x - 4. * j, scale=5.).rvs()
            if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x - 4. * j, scale=5.).pdf(Y) / 2.):
                return Y
        else:
            Y = norm(loc=x + 4. * j, scale=5.).rvs()
            if U < trial_proposal(Y, x, j, M) / (upper_bound * norm(loc=x + 4. * j, scale=5.).pdf(Y) / 2.):
